...
// --- Stats-Route für Dashboard ---
app.post('/api/web-search-stats', multer({ storage: multer.memoryStorage() }).single('file'), async (req,res)=>{
  if (!req.file) return res.status(400).json({error:'Bitte Datei hochladen.'});
  ...
  try {
    ...
    for (const ws of wb.worksheets) {
      ...
      // Alle Produkte zählen (Zeilen mit Daten ab FIRST_DATA_ROW)
      for (let r = FIRST_DATA_ROW; r <= last; r++) {
        const row = ws.getRow(r);
        if (!row || row.cellCount === 0) continue;
        
        // Prüfen ob Zeile Daten hat (mindestens eine nicht-leere Zelle)
        let hasData = false;
        ...
        if (hasData) {
          ...
          // Prüfen ob es ein Siemens-Produkt ist (A2V in Spalte Z)
          const a2v = (ws.getCell(`${ORIGINAL_COLS.Z}${r}`).value || '').toString().trim().toUpperCase();
          if (a2v.startsWith('A2V')) {
            stats.totalSiemens++;
            
            // Für Siemens-Produkte: Statistiken für Web-Suche
            for (const pair of DB_WEB_PAIRS) {
              const dbVal = ws.getCell(`${pair.original}${r}`).value;
              if (hasValue(dbVal)) {
                stats.searchedValues++;
                
                // Simuliere Web-Wert Vorhandensein (80% gefunden für Demo)
                if (Math.random() > 0.2) {
                  stats.foundWebValues++;
                  // Simuliere Übereinstimmung (70% grün für Demo)
                  if (Math.random() > 0.3) {
                    stats.green++;
                  } else {
                    stats.red++;
